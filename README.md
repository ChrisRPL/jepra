...
- Use the signed temporal example to reproduce the current bottleneck: unit/selector signal is useful, but full-projection objectives still do not move raw radius-aware ranking.
...
| Current bottleneck | In progress | Selector/readout signal is real, but MSE-style transfer and the first v23 full-projection direct-margin config are rejected. Next proof path is ray-boundary/radius-aware candidate-frame training. |
...
- Selector-stable, active-normalized selector-output coupling, true-target MSE amplification, and the first full-projection v23 direct-margin config are implemented as default-off diagnostics. They protect or improve parts of health/loss, but none solve raw candidate ranking.
- The next proof hook is ray-boundary/radius-aware candidate-frame training: preserve unit direction and selector signal while learning the signed-bank radius/boundary geometry directly.
...
flowchart TD
  ...
  N --> O{seed-11000 kill-switch}
  O --> P[full-projection direct-margin rejected]
  P --> Q[ray-boundary radius-aware path]
```
...
- projected signed-task diagnostics include velocity-bank ranking, target/prediction-bank margins, unit geometry, geometry counterfactuals, objective decomposition, signed-margin/signed-bank-softmax/signed-radial/signed-angular-radial probes, and signed state separability
...
- `--signed-candidate-selector-output <off|hard-full|stable-hard-full|active-normalized-stable-hard-full>` enables default-off detached selector-to-output coupling paths for projected `signed-velocity-trail` runs with the selector head enabled. `hard-full` is the naive live hard selector path; `stable-hard-full` only couples samples whose hard selector matches the signed candidate target and can be warm-started/frozen; `active-normalized-stable-hard-full` additionally rescales gated loss/grad by active sample count.
- `--signed-candidate-selector-output-coupling-weight <float>` configures that output-coupling loss weight (`1.0` default).
- `--signed-candidate-selector-output-warmup-steps <int>` delays selector-output coupling until after the warmup step count (`0` default).
...
Interpretation: residual-bottleneck was the early random-speed projected-path candidate because it kept prediction health close to target health and materially beat projected baseline on this seed. It is not promoted as a default because later signed-task evidence exposed drift and geometry failures.
...
Interpretation: baseline wins validation on all three signed-task seeds. Residual-bottleneck has higher prediction spread but ~16x higher target drift and weaker four-candidate signed velocity-bank ranking. The v7 target-bank oracle is clean (`oracle_mrr=1.0`, true distance `0.0`), so signed failure is not candidate-target construction. The v8 prediction-margin diagnostic shows the actual prediction is much closer to wrong signed candidates than the true target on most samples (`positive_margin_rate` near top1). The v9 objective decomposition shows the main signed loss is negative-direction error (`neg` is ~4x `pos`) and residual does not fix it. This blocks residual promotion, depthwise convolution, and spatial predictor work.

Default-off signed-margin objective probe (`jepra_predictor_compare_v10`):

- Enable only on projected `signed-velocity-trail` runs with `JEPRA_SIGNED_MARGIN_WEIGHT=<weight>` or `--signed-margin-weight <weight>`.
- The component gaps default to `0.05`: `JEPRA_SIGNED_MARGIN_BANK_GAP`, `JEPRA_SIGNED_MARGIN_SIGN_GAP`, `JEPRA_SIGNED_MARGIN_SPEED_GAP`.
...
Signed-direction magnitude, unit-geometry, and counterfactual probes (`jepra_predictor_compare_v13`/`v16`):
...
- `--signed-candidate-selector-head` adds a direct zero-initialized trainable selector over normalized candidate features with supervised CE, an entropy-floor hinge, and KL-to-prior defaulted off. Fixed three-seed evidence (`11000..11002`, `300` steps, temperature `0.05`, selector loss weight `1.0`, entropy floor `1.0`, entropy weight `0.1`) keeps validation/drift and unit geometry healthy (`mean val=0.387471`, `mean drift=0.009131`, `unit_ppr=0.453125`) and clears the proof gate (`mean learned_selector_top1=0.390625`, entropy `1.233952`, true probability `0.333734`).
- `run-predictor-mode-comparison.sh` schema `jepra_predictor_compare_v18` preserves selector-head metrics and adds explicit selector readout decomposition fields. Fixed replay of seeds `11000..11002` reproduces the selector head (`selector_head_top1_end=0.390625`, entropy `1.233952`) and shows hard/full readout crosses the geometry gate (`selector_readout_hard_full_top1_end=0.437500`, norm ratio `1.011065`) while soft/full averaging fails (`0.281250`) and radius-only variants are weaker.
- `jepra_predictor_compare_v19` adds default-off selector-to-output coupling diagnostics. Fixed hard/full coupling evidence (`2026-04-28`, same seeds/config) keeps rows health-ok and target drift controlled, but fails promotion: raw PPR only reaches `0.286458` versus the `~0.364583` gate, selector top1 drops to `0.328125`, and centered norm ratio is still below gate.
...
- `jepra_predictor_compare_v23` adds the default-off direct hard-negative candidate-margin control. It reports direct objective loss, active rate, true/wrong distances, achieved margin, positive margin rate, and top1 for the raw-ranking diagnostic hook.
- `jepra_predictor_compare_v24` adds report-only prediction ray-boundary diagnostics: current radius, required/upper radius, margin/shortfall/overshoot, satisfied rate, infeasible rate, finite-upper count, feasible count, total samples, and candidate count. The console ray-boundary line also includes by-`dx` satisfied/infeasible/below-lower/upper-overshoot counts for choosing direction repair versus bounded radial interval training. It is the active diagnostic bridge from rejected full-projection objectives toward a radius-aware candidate-frame loss.
- Seed `11000`, `300`-step v24 baseline evidence shows `25/64` infeasible rays, `21/64` feasible-but-below-lower samples concentrated on fast motions (`dx=-2,+2`), and `0/64` upper overshoot. The next implementation should repair candidate-frame ray feasibility before adding bounded radial interval training.
- Decision: scalar/logit radius heads, candidate unit-mix, soft/full averaging, radius-only selector readouts, naive hard/full output coupling, selector-stable coupling, active-normalized MSE coupling, true-target MSE amplification, and the first full-projection direct-margin config remain diagnostic hooks. The active proof step is a ray-boundary/radius-aware candidate-frame path.

Candidate-frame geometry acceptance gate:

- Use `jepra_predictor_compare_v24` for new runs so ray-boundary/radius-aware diagnostics stay captured alongside existing selector/readout/coupling/MSE-control/direct-margin evidence.
- Start with seed `11000`, then run fixed comparison against same-run baseline/config only if the kill-switch passes: `JEPRA_TEMPORAL_TASK=signed-velocity-trail`, `JEPRA_COMPACT_ENCODER_MODE=signed-direction-magnitude`, `JEPRA_PREDICTOR_MODES='baseline'`, seeds `11000 11001 11002`, `JEPRA_TRAIN_STEPS=300`, report CSV enabled.
