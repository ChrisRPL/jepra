...
- Decision: keep the scalar residual head as a diagnostic/training hook, not the final fix. Next implementation should use candidate-logit residual radius mixing in the same centroid frame: preserve bank-centered direction, learn residual logits over candidate radii, and train with the centered-radius scalar primitive gradient.
