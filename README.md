...
```text
seed=<seed> momentum=<1.0|0.5|0.0> profile=<warmup|frozen|trainable|zero> status=ok | projected run summary | steps 80 | train <start> -> <end> (Δ <delta>, improved=<true|false>) | val <start> -> <end> (Δ <delta>, improved=<true|false>) | drift <drift>
```

Latest fixed-seed summary lives in [Projected Momentum Evidence](agent-wiki/06-projected-momentum-sweep-evidence.md) and is used as phase evidence for the projected hardening gate.
