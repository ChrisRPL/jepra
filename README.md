...
- Use the signed temporal example to reproduce the current bottleneck: hard/full selector readout is useful, but direct selector-to-output coupling still does not move raw prediction geometry.
...
| Current bottleneck | In progress | Selector/readout signal is real, but MSE-style selector-to-output coupling still does not transfer it into raw base PPR; next work separates MSE underpowering from objective mismatch. |
...
- Selector-stable and active-normalized selector-output coupling are implemented as default-off diagnostics. They protect health and selector signal, but base raw PPR remains pinned, so the next proof step is an MSE-amplification control followed by candidate-margin loss if MSE still fails.
...
flowchart TD
  ...
  K --> L{stabilize selector before coupling}
```
...
- Decision: scalar/logit radius heads, candidate unit-mix, soft/full averaging, radius-only selector readouts, naive hard/full output coupling, selector-stable coupling, and active-normalized MSE coupling remain diagnostic hooks. The next selector-output step should test whether true-target MSE amplification can move raw PPR; if not, move to direct candidate-margin loss.

Candidate-centroid-aware geometry acceptance gate:

- Use the existing `jepra_predictor_compare_v21` fields for current selector/readout/coupling evidence. Only bump the schema again if a future patch emits new selector-to-output coupling diagnostics that cannot be derived from current prediction-bank/unit/counterfactual/selector/readout/coupling fields.
