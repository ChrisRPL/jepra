...
SCHEMA="jepra_predictor_compare_v24"
...
SIGNED_CANDIDATE_SELECTOR_OUTPUT_WARMUP_STEPS="${JEPRA_SIGNED_CANDIDATE_SELECTOR_OUTPUT_WARMUP_STEPS:-0}"
SIGNED_CANDIDATE_SELECTOR_OUTPUT_FREEZE_SELECTOR_AFTER_WARMUP="${JEPRA_SIGNED_CANDIDATE_SELECTOR_OUTPUT_FREEZE_SELECTOR_AFTER_WARMUP:-0}"
SIGNED_CANDIDATE_SELECTOR_OUTPUT_MIN_CONFIDENCE="${JEPRA_SIGNED_CANDIDATE_SELECTOR_OUTPUT_MIN_CONFIDENCE:-0.0}"
...
Environment:
  ...
  JEPRA_SIGNED_CANDIDATE_SELECTOR_OUTPUT_FREEZE_SELECTOR_AFTER_WARMUP Freeze selector after warmup (default: 0)
  JEPRA_SIGNED_CANDIDATE_SELECTOR_OUTPUT_MIN_CONFIDENCE Selector output minimum max probability gate (default: 0.0)
...
signed_true_target_mse_amplification_enabled() {
  awk -v value="$SIGNED_TRUE_TARGET_MSE_AMPLIFICATION_WEIGHT" 'BEGIN { if (value + 0 > 0) print "true"; else print "false" }'
}
...
signed_candidate_selector_output_coupling_enabled() {
  if [[ "$(signed_candidate_selector_output_mode)" != "off" ]]; then
    printf '%s' "true"
    return
  fi
  printf '%s' "false"
...
parse_prediction_ray_boundary_line() {
  local line="$1"
  if [[ ! "$line" =~ ^(initial|final)[[:space:]]+\|[[:space:]]+signed[[:space:]]+prediction[[:space:]]+ray[[:space:]]+boundary[[:space:]]+current_radius[[:space:]] ]]; then
    return 1
  fi
  awk '
    {
      ...
      print values["current_radius"], values["required_radius"], values["upper_radius"], values["radius_margin"], values["radius_shortfall"], values["radius_overshoot"], values["satisfied_rate"], values["infeasible_rate"], values["finite_upper_samples"], values["feasible_samples"], values["samples"], values["candidates"]
    }
  ' <<< "$line"
}
...
parse_signed_direct_candidate_margin_objective_line() {
  local line="$1"
  if [[ ! "$line" =~ ^(initial|final)[[:space:]]+\|[[:space:]]+signed[[:space:]]+direct[[:space:]]+candidate-margin[[:space:]]+objective[[:space:]]+loss[[:space:]] ]]; then
    return 1
  fi
  awk '
    {
      ...
      print values["loss"], values["active_rate"], values["true_distance"], values["wrong_distance"], values["margin"], values["positive_margin_rate"], values["top1"], values["active_count"], values["samples"]
    }
  ' <<< "$line"
}
  ...
  awk '
    {
      ...
      print values["loss"], values["selector_max_probability"], values["active_rate"], values["base_prediction_top1"], values["base_prediction_margin"], values["base_prediction_norm_ratio"], values["selector_hard_full_top1"], values["selector_hard_full_margin"], values["selector_hard_full_norm_ratio"], values["active_samples"], values["samples"], values["candidates"]
    }
  ' <<< "$line"
}
...
emit_row() {
  ...
  local prediction_ray_current_radius_end_value="${prediction_ray_current_radius_end:-na}"
  local prediction_ray_required_radius_end_value="${prediction_ray_required_radius_end:-na}"
  local prediction_ray_upper_radius_end_value="${prediction_ray_upper_radius_end:-na}"
  local prediction_ray_radius_margin_end_value="${prediction_ray_radius_margin_end:-na}"
  local prediction_ray_radius_shortfall_end_value="${prediction_ray_radius_shortfall_end:-na}"
  local prediction_ray_radius_overshoot_end_value="${prediction_ray_radius_overshoot_end:-na}"
  local prediction_ray_satisfied_rate_end_value="${prediction_ray_satisfied_rate_end:-na}"
  local prediction_ray_infeasible_rate_end_value="${prediction_ray_infeasible_rate_end:-na}"
  local prediction_ray_finite_upper_samples_value="${prediction_ray_finite_upper_samples:-na}"
  local prediction_ray_feasible_samples_value="${prediction_ray_feasible_samples:-na}"
  local prediction_ray_samples_value="${prediction_ray_samples:-na}"
  local prediction_ray_candidates_value="${prediction_ray_candidates:-na}"
  ...
  local signed_direct_candidate_margin_loss_end_value="${signed_direct_candidate_margin_loss_end:-na}"
  local signed_direct_candidate_margin_active_rate_end_value="${signed_direct_candidate_margin_active_rate_end:-na}"
  local signed_direct_candidate_margin_true_distance_end_value="${signed_direct_candidate_margin_true_distance_end:-na}"
  local signed_direct_candidate_margin_wrong_distance_end_value="${signed_direct_candidate_margin_wrong_distance_end:-na}"
  local signed_direct_candidate_margin_margin_end_value="${signed_direct_candidate_margin_margin_end:-na}"
  local signed_direct_candidate_margin_positive_margin_rate_end_value="${signed_direct_candidate_margin_positive_margin_rate_end:-na}"
  local signed_direct_candidate_margin_top1_end_value="${signed_direct_candidate_margin_top1_end:-na}"
  local signed_direct_candidate_margin_active_count_end_value="${signed_direct_candidate_margin_active_count_end:-na}"
  local signed_direct_candidate_margin_samples_value="${signed_direct_candidate_margin_samples:-na}"
  ...
  local selector_output_coupling_active_rate_end_value="${selector_output_coupling_active_rate_end:-na}"
  ...
  local selector_output_coupling_active_samples_value="${selector_output_coupling_active_samples:-na}"
  ...
  printf 'schema=%s temporal_task=%s path=%s predictor=%s residual_delta_scale=%s projector_drift_weight=%s signed_margin_weight=%s signed_margin_bank_gap=%s signed_margin_sign_gap=%s signed_margin_speed_gap=%s signed_margin_bank_weight=%s signed_margin_sign_weight=%s signed_margin_speed_weight=%s signed_bank_softmax_weight=%s signed_bank_softmax_temperature=%s signed_radial_weight=%s signed_angular_radial_weight=%s signed_angular_weight=%s signed_angular_radial_radius_weight=%s signed_true_target_mse_amplification_weight=%s signed_direct_candidate_margin_weight=%s signed_direct_candidate_margin=%s seed=%s steps=%s encoder_mode=%s encoder_lr=%s target_momentum_start=%s target_momentum_end=%s target_momentum_warmup_steps=%s train_pred_start=%s train_pred_end=%s val_pred_start=%s val_pred_end=%s train_obj_start=%s train_obj_end=%s val_obj_start=%s val_obj_end=%s pred_min_std_final=%s target_min_std_final=%s proj_var_mean_final=%s target_drift_end=%s velocity_bank_mrr_start=%s velocity_bank_mrr_end=%s velocity_bank_top1_start=%s velocity_bank_top1_end=%s velocity_bank_mean_rank_start=%s velocity_bank_mean_rank_end=%s velocity_bank_samples=%s velocity_bank_candidates=%s signed_bank_neg_mrr_end=%s signed_bank_pos_mrr_end=%s signed_bank_slow_mrr_end=%s signed_bank_fast_mrr_end=%s signed_bank_sign_top1_end=%s signed_bank_speed_top1_end=%s signed_bank_samples=%s signed_bank_true_neg_best_neg=%s signed_bank_true_neg_best_pos=%s signed_bank_true_pos_best_neg=%s signed_bank_true_pos_best_pos=%s signed_bank_true_slow_best_slow=%s signed_bank_true_slow_best_fast=%s signed_bank_true_fast_best_slow=%s signed_bank_true_fast_best_fast=%s target_bank_oracle_mrr_end=%s target_bank_oracle_top1_end=%s target_bank_true_distance_end=%s target_bank_true_distance_max_end=%s target_bank_nearest_wrong_end=%s target_bank_nearest_wrong_min_end=%s target_bank_margin_end=%s target_bank_margin_min_end=%s target_bank_neg_nearest_wrong_end=%s target_bank_pos_nearest_wrong_end=%s target_bank_slow_nearest_wrong_end=%s target_bank_fast_nearest_wrong_end=%s target_bank_sign_margin_end=%s target_bank_speed_margin_end=%s target_bank_samples=%s prediction_bank_true_distance_end=%s prediction_bank_nearest_wrong_distance_end=%s prediction_bank_margin_end=%s prediction_bank_min_margin_end=%s prediction_bank_positive_margin_rate_end=%s prediction_bank_sign_margin_end=%s prediction_bank_speed_margin_end=%s prediction_bank_samples=%s prediction_unit_mrr_end=%s prediction_unit_top1_end=%s prediction_unit_true_distance_end=%s prediction_unit_nearest_wrong_distance_end=%s prediction_unit_margin_end=%s prediction_unit_positive_margin_rate_end=%s prediction_unit_sign_margin_end=%s prediction_unit_speed_margin_end=%s prediction_unit_prediction_center_norm_end=%s prediction_unit_true_target_center_norm_end=%s prediction_unit_samples=%s prediction_unit_candidates=%s prediction_ray_current_radius_end=%s prediction_ray_required_radius_end=%s prediction_ray_upper_radius_end=%s prediction_ray_radius_margin_end=%s prediction_ray_radius_shortfall_end=%s prediction_ray_radius_overshoot_end=%s prediction_ray_satisfied_rate_end=%s prediction_ray_infeasible_rate_end=%s prediction_ray_finite_upper_samples=%s prediction_ray_feasible_samples=%s prediction_ray_samples=%s prediction_ray_candidates=%s prediction_counterfactual_oracle_radius_mrr_end=%s prediction_counterfactual_oracle_radius_top1_end=%s prediction_counterfactual_oracle_radius_margin_end=%s prediction_counterfactual_oracle_radius_positive_margin_rate_end=%s prediction_counterfactual_oracle_radius_sign_margin_end=%s prediction_counterfactual_oracle_radius_speed_margin_end=%s prediction_counterfactual_oracle_radius_norm_ratio_end=%s prediction_counterfactual_oracle_angle_mrr_end=%s prediction_counterfactual_oracle_angle_top1_end=%s prediction_counterfactual_oracle_angle_margin_end=%s prediction_counterfactual_oracle_angle_positive_margin_rate_end=%s prediction_counterfactual_oracle_angle_sign_margin_end=%s prediction_counterfactual_oracle_angle_speed_margin_end=%s prediction_counterfactual_oracle_angle_norm_ratio_end=%s prediction_counterfactual_support_global_rescale_mrr_end=%s prediction_counterfactual_support_global_rescale_top1_end=%s prediction_counterfactual_support_global_rescale_margin_end=%s prediction_counterfactual_support_global_rescale_positive_margin_rate_end=%s prediction_counterfactual_support_global_rescale_sign_margin_end=%s prediction_counterfactual_support_global_rescale_speed_margin_end=%s prediction_counterfactual_support_global_rescale_norm_ratio_end=%s prediction_counterfactual_support_norm_ratio_end=%s prediction_counterfactual_support_samples=%s prediction_counterfactual_query_samples=%s prediction_counterfactual_candidates=%s signed_objective_all_loss_end=%s signed_objective_dx_neg2_loss_end=%s signed_objective_dx_neg1_loss_end=%s signed_objective_dx_pos1_loss_end=%s signed_objective_dx_pos2_loss_end=%s signed_objective_neg_loss_end=%s signed_objective_pos_loss_end=%s signed_objective_slow_loss_end=%s signed_objective_fast_loss_end=%s signed_objective_sign_gap_end=%s signed_objective_speed_gap_end=%s signed_objective_samples=%s signed_objective_dx_neg2_samples=%s signed_objective_dx_neg1_samples=%s signed_objective_dx_pos1_samples=%s signed_objective_dx_pos2_samples=%s signed_margin_bank_loss_end=%s signed_margin_sign_loss_end=%s signed_margin_speed_loss_end=%s signed_margin_weighted_loss_end=%s signed_margin_active_bank_rate_end=%s signed_margin_active_sign_rate_end=%s signed_margin_active_speed_rate_end=%s signed_margin_samples=%s signed_bank_softmax_loss_end=%s signed_bank_softmax_top1_end=%s signed_bank_softmax_true_probability_end=%s signed_bank_softmax_samples=%s signed_radial_loss_end=%s signed_radial_prediction_norm_end=%s signed_radial_target_norm_end=%s signed_radial_norm_ratio_end=%s signed_radial_samples=%s signed_angular_radial_loss_end=%s signed_angular_radial_angular_loss_end=%s signed_angular_radial_radial_loss_end=%s signed_angular_radial_cosine_end=%s signed_angular_radial_prediction_norm_end=%s signed_angular_radial_target_norm_end=%s signed_angular_radial_norm_ratio_end=%s signed_angular_radial_samples=%s state_latent_mrr_end=%s state_latent_top1_end=%s state_latent_sign_top1_end=%s state_latent_mean_rank_end=%s state_projection_mrr_end=%s state_projection_top1_end=%s state_projection_sign_top1_end=%s state_projection_mean_rank_end=%s state_support_samples=%s state_query_samples=%s state_candidates=%s selector_head_enabled=%s selector_head_temperature=%s selector_head_lr=%s selector_head_weight=%s selector_head_entropy_floor=%s selector_head_entropy_weight=%s selector_head_kl_weight=%s selector_head_mrr_end=%s selector_head_top1_end=%s selector_head_margin_end=%s selector_head_positive_margin_rate_end=%s selector_head_sign_margin_end=%s selector_head_speed_margin_end=%s selector_head_norm_ratio_end=%s selector_head_objective_loss_end=%s selector_head_objective_ce_end=%s selector_head_objective_entropy_reg_end=%s selector_head_objective_kl_to_prior_end=%s selector_head_entropy_end=%s selector_head_true_probability_end=%s selector_head_max_probability_end=%s selector_head_temperature_end=%s selector_head_steps=%s selector_head_lr_end=%s selector_head_entropy_floor_end=%s selector_head_entropy_weight_end=%s selector_head_kl_weight_end=%s selector_head_support_samples=%s selector_head_query_samples=%s selector_head_candidates=%s status=%s\n' \
    ...
    "$SIGNED_TRUE_TARGET_MSE_AMPLIFICATION_WEIGHT" "$SIGNED_DIRECT_CANDIDATE_MARGIN_WEIGHT" "$SIGNED_DIRECT_CANDIDATE_MARGIN" \
    ...
    "$prediction_ray_current_radius_end_value" "$prediction_ray_required_radius_end_value" "$prediction_ray_upper_radius_end_value" \
    "$prediction_ray_radius_margin_end_value" "$prediction_ray_radius_shortfall_end_value" "$prediction_ray_radius_overshoot_end_value" \
    "$prediction_ray_satisfied_rate_end_value" "$prediction_ray_infeasible_rate_end_value" "$prediction_ray_finite_upper_samples_value" \
    "$prediction_ray_feasible_samples_value" "$prediction_ray_samples_value" "$prediction_ray_candidates_value" \
  ...
  if [[ -n "$REPORT_PATH" ]]; then
    local row_values=(
      ...
      "$signed_margin_weighted_loss_end_value" "$signed_margin_active_bank_rate_end_value" "$signed_margin_active_sign_rate_end_value" \
      "$signed_margin_active_speed_rate_end_value" "$signed_margin_samples_value" \
      "$signed_direct_candidate_margin_loss_end_value" "$signed_direct_candidate_margin_active_rate_end_value" \
      "$signed_direct_candidate_margin_true_distance_end_value" "$signed_direct_candidate_margin_wrong_distance_end_value" \
      "$signed_direct_candidate_margin_margin_end_value" "$signed_direct_candidate_margin_positive_margin_rate_end_value" \
      "$signed_direct_candidate_margin_top1_end_value" "$signed_direct_candidate_margin_active_count_end_value" \
      "$signed_direct_candidate_margin_samples_value" \
      "$signed_bank_softmax_loss_end_value" "$signed_bank_softmax_top1_end_value" "$signed_bank_softmax_true_probability_end_value" \
      "$signed_bank_softmax_samples_value" \
      "$signed_radial_loss_end_value" "$signed_radial_prediction_norm_end_value" "$signed_radial_target_norm_end_value" \
      "$signed_radial_norm_ratio_end_value" "$signed_radial_samples_value" \
      ...
      "$SIGNED_CANDIDATE_SELECTOR_OUTPUT_COUPLING_WEIGHT" "$SIGNED_CANDIDATE_SELECTOR_OUTPUT_WARMUP_STEPS" \
      "$SIGNED_CANDIDATE_SELECTOR_OUTPUT_FREEZE_SELECTOR_AFTER_WARMUP" "$SIGNED_CANDIDATE_SELECTOR_OUTPUT_MIN_CONFIDENCE" \
      "$selector_output_coupling_loss_end_value" "$selector_output_coupling_selector_max_probability_end_value" \
      "$selector_output_coupling_active_rate_end_value" "$selector_output_coupling_base_prediction_top1_end_value" \
      ...
      "$selector_output_coupling_hard_full_norm_ratio_end_value" "$selector_output_coupling_active_samples_value" "$selector_output_coupling_samples_value" \
...
run_one() {
  ...
  local prediction_ray_current_radius_end="na"
  local prediction_ray_required_radius_end="na"
  local prediction_ray_upper_radius_end="na"
  local prediction_ray_radius_margin_end="na"
  local prediction_ray_radius_shortfall_end="na"
  local prediction_ray_radius_overshoot_end="na"
  local prediction_ray_satisfied_rate_end="na"
  local prediction_ray_infeasible_rate_end="na"
  local prediction_ray_finite_upper_samples="na"
  local prediction_ray_feasible_samples="na"
  local prediction_ray_samples="na"
  local prediction_ray_candidates="na"
  ...
  local signed_direct_candidate_margin_loss_end="na"
  local signed_direct_candidate_margin_active_rate_end="na"
  local signed_direct_candidate_margin_true_distance_end="na"
  local signed_direct_candidate_margin_wrong_distance_end="na"
  local signed_direct_candidate_margin_margin_end="na"
  local signed_direct_candidate_margin_positive_margin_rate_end="na"
  local signed_direct_candidate_margin_top1_end="na"
  local signed_direct_candidate_margin_active_count_end="na"
  local signed_direct_candidate_margin_samples="na"
  ...
  local selector_output_coupling_active_rate_end="na"
  ...
  local selector_output_coupling_active_samples="na"
  ...
  if [[ "$(signed_true_target_mse_amplification_enabled)" == "true" ]]; then
    extra_args+=(--signed-true-target-mse-amplification-weight "$SIGNED_TRUE_TARGET_MSE_AMPLIFICATION_WEIGHT")
  fi
  ...
  if [[ "$(signed_candidate_selector_output_coupling_enabled)" == "true" ]]; then
    extra_args+=(--signed-candidate-selector-output "$(signed_candidate_selector_output_mode)")
    extra_args+=(--signed-candidate-selector-output-coupling-weight "$SIGNED_CANDIDATE_SELECTOR_OUTPUT_COUPLING_WEIGHT")
    extra_args+=(--signed-candidate-selector-output-warmup-steps "$SIGNED_CANDIDATE_SELECTOR_OUTPUT_WARMUP_STEPS")
    extra_args+=(--signed-candidate-selector-output-min-confidence "$SIGNED_CANDIDATE_SELECTOR_OUTPUT_MIN_CONFIDENCE")
    case "$SIGNED_CANDIDATE_SELECTOR_OUTPUT_FREEZE_SELECTOR_AFTER_WARMUP" in
      1|true|TRUE|yes|YES|on|ON)
        extra_args+=(--signed-candidate-selector-output-freeze-selector-after-warmup)
        ;;
    esac
  fi
  ...
  local final_prediction_ray_boundary_line
  ...
  local final_signed_direct_candidate_margin_objective_line
  ...
  if [[ "$path" == "unprojected" ]]; then
    ...
    if is_signed_velocity_bank_task; then
      ...
      final_prediction_ray_boundary_line="$(grep -m1 '^final | signed prediction ray boundary current_radius ' "$log_file" || true)"

      if ! parsed="$(parse_prediction_ray_boundary_line "$final_prediction_ray_boundary_line")"; then
        failures=$((failures + 1))
        emit_row "$path" "$predictor" "$seed" "$encoder_lr" "$target_momentum_start" "$target_momentum_end" "$target_momentum_warmup_steps" "$train_pred_start" "$train_pred_end" "$val_pred_start" "$val_pred_end" "$train_obj_start" "$train_obj_end" "$val_obj_start" "$val_obj_end" "na" "na" "$proj_var_mean" "$target_drift_end" "parse_failed"
        rm -f "$log_file"
        return 0
      fi
      read -r prediction_ray_current_radius_end prediction_ray_required_radius_end prediction_ray_upper_radius_end prediction_ray_radius_margin_end prediction_ray_radius_shortfall_end prediction_ray_radius_overshoot_end prediction_ray_satisfied_rate_end prediction_ray_infeasible_rate_end prediction_ray_finite_upper_samples prediction_ray_feasible_samples prediction_ray_samples prediction_ray_candidates <<< "$parsed"
      ...
      if [[ "$(signed_direct_candidate_margin_enabled)" == "true" ]]; then
        final_signed_direct_candidate_margin_objective_line="$(grep -m1 '^final | signed direct candidate-margin objective loss ' "$log_file" || true)"

        if ! parsed="$(parse_signed_direct_candidate_margin_objective_line "$final_signed_direct_candidate_margin_objective_line")"; then
          failures=$((failures + 1))
          emit_row "$path" "$predictor" "$seed" "$encoder_lr" "$target_momentum_start" "$target_momentum_end" "$target_momentum_warmup_steps" "$train_pred_start" "$train_pred_end" "$val_pred_start" "$val_pred_end" "$train_obj_start" "$train_obj_end" "$val_obj_start" "$val_obj_end" "na" "na" "$proj_var_mean" "$target_drift_end" "parse_failed"
          rm -f "$log_file"
          return 0
        fi
        read -r signed_direct_candidate_margin_loss_end signed_direct_candidate_margin_active_rate_end signed_direct_candidate_margin_true_distance_end signed_direct_candidate_margin_wrong_distance_end signed_direct_candidate_margin_margin_end signed_direct_candidate_margin_positive_margin_rate_end signed_direct_candidate_margin_top1_end signed_direct_candidate_margin_active_count_end signed_direct_candidate_margin_samples <<< "$parsed"
      fi
