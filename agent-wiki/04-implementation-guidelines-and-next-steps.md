...
- Candidate-radius scalar residual head (`2026-04-25`, seed `11000`, `signed-direction-magnitude`, baseline, `300` steps): `--signed-candidate-radius-head` with temperature `0.05` keeps validation/drift healthy and improves radius calibration (`learned_radius_norm_ratio=1.164618` versus report-only softmax `1.379775`), but top1 remains `0.328125`, below the `~0.364583` proof gate. Temperature `0.25` worsens top1 to `0.250000`.
- Candidate-radius logit residual head (`2026-04-26`, seed `11000`, `signed-direction-magnitude`, baseline, `300` steps): `--signed-candidate-radius-logit-head` zero-initializes residual logits over candidate radii, matches report-only softmax at initialization, and trains only the logit head. Validation/drift stay healthy, but the kill-switch fails: `learned_radius_top1=0.343750`, matching report-only softmax on this seed and below the `~0.364583` proof gate; `learned_radius_norm_ratio=1.540515` also fails the `[0.50, 1.50]` radius bound.
- Candidate selector head (`2026-04-26`, seed `11000`, `signed-direction-magnitude`, baseline, `300` steps): `--signed-candidate-selector-head` uses direct selector logits, normalized candidate features, CE, an entropy-floor hinge, and KL-to-prior defaulted off. Validation/drift stay healthy (`val=0.387588`, `drift=0.009244`), unit geometry stays strong (`unit_ppr=0.453125`), and the selector clears the single-seed proof gate (`learned_selector_top1=0.390625`, entropy `1.237072`, true probability `0.331317`).
- Decision: signed evidence still blocks residual promotion, depthwise convolution, and spatial predictor work. Target-bank construction is valid, prediction-bank margins show predictions closer to wrong signed futures than true targets on most samples, and radius-only paths were insufficient. The direct entropy-floor selector is now the active candidate-centered trainable path; next valid step is a three-seed validation, not another radius-only sweep or CUDA/paper detour.
...
- The direct entropy-floor selector head fixes the single-seed collapse failure (`learned_selector_top1=0.390625`, entropy `1.237072`), so the next implementation step is validation across seeds `11000..11002` before promotion.
